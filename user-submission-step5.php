<?php
/*
 ** Template Name: User Submission Step 5
 */
...
if (isset($_POST['currentStep']) && $_POST['currentStep'] == 5) {
	$updated = update_user_meta($user_ID, 'current_step', $_POST['currentStep']);
	update_post_meta($post_id, 'legal_agreement', $_POST['legalAgreement']);
	if (isset($_POST['next'])) {
		wp_redirect(home_url('user-submission-step6'));
	}
}

if ($user_ID) {
	$user_current_step = get_user_meta($user_ID, 'current_step', true);
	$legal = get_field("legal_agreement", $post_id);
?>
		...
		<div class="submission-form">
			...
			<div class="submission-step active">
				<div class="steps-progress">
					<h3>Legal document & Competitive agreement</h3>
				...
				$file_download = $upload_dir['baseurl'] . "/pdf/mytest.pdf";
				?>
				<div class="document-requirements judging-aspects">
					<h3>Leagal Document</h3>
					...
					<a href="<?php echo $file_download; ?>" class="text-uppercase">Download this document</a>
				</div>
				<div class="document-requirements">
					<h3>Competitive Agreement</h3>
					<p>we will judge your documents base on the following areas. Atomorum voluptatum definitionem per. Zril petentium sit at, vel at quis corrumpit. At facilisi contentiones per. Dicta omnes ius ut, ei atomorum voluptatum definitionem per. Zril petentium sit at, vel at quis corrumpit. At facilisi contentiones per.</p>
					<a href="<?php echo $file_download; ?>" class="text-uppercase">Download this document</a>
				</div>
				<form id="step5-form" action="" method="post">
					<div class="document-agreement">
						<div class="form-check">
							<input type="checkbox" name="legalAgreement" id="legalAgreement" class="form-check-input" value="Yes"<?php if ($legal == "Yes") echo ' checked'; ?>>
							<label class="form-check-label">I understand by clicking next, I will agree to this legal document.</label>
						</div>
					</div>
...
jQuery(document).ready(function($) {
	...
	$("#legalAgreement").change(function() {
	...
	$('#cancelModal button[type=submit]').click(function(e) {
		window.location.href = '/user-submission-step4/';
	});

	$("#next-btn").click(function() {
		location.href = 'user-submission-step6';
	});
});
</script>
<?php
